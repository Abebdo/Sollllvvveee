...
export class RiskEngine {
  static async assess(input: string, type: InputType): Promise<RiskAssessment> {
    ...
    try {
        ...
        if (healthData.status !== 'ok') throw new Error('Backend reported unhealthy status');
        console.log('[RiskEngine] Backend Health Confirmed:', healthData);
    } catch (e) {
        console.error('[RiskEngine] Backend Unreachable:', e);
        return {
            status: 'NO_ANALYSIS',
            risk_level: 'Minimal',
