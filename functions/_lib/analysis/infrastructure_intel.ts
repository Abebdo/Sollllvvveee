...
export function analyzeInfrastructure(
    artifact: string,
    type: string,
    intent: 'BENIGN' | 'SUSPICIOUS' | 'MALICIOUS' = 'BENIGN'
): InfrastructureIntelResult {
    let risk = 0;
    ...
    if (lower.includes('docs.google.com') || lower.includes('forms.office.com') || lower.includes('forms.gle') || lower.includes('sharepoint.com')) {
    ...
    // 4. CDN Fronting Abuse (Generic Check)
    ...
    if (cdnDomains.some(d => lower.includes(d))) {
        ...
        risk += 10;
         if (type === 'url' && (lower.endsWith('.exe') || lower.endsWith('.zip') || lower.endsWith('.pdf') || lower.endsWith('.iso'))) {
             risk += 40;
             trustedAbuse = true;
    ...
    // --- PHASE 1: Trusted Infrastructure + Malicious Intent ---
    // If the provider is trusted (known cloud provider or productivity suite) AND intent is Malicious/Suspicious
    if ((match || provider === 'Trusted Productivity Suite') && (intent === 'MALICIOUS' || intent === 'SUSPICIOUS')) {
        risk += 60; // Significant penalty
        trustedAbuse = true;
        abuseType = 'Trusted infrastructure abused to inherit false legitimacy';
    }
